...
docker compose exec -T drupal bash -lc 'curl -sS https://getcomposer.org/installer | php -- --install-dir=/usr/local/bin --filename=composer && composer --version'
...
echo "--- Install Drupal ---"
/usr/bin/env docker compose exec -T drupal bash -lc "/opt/drupal/vendor/bin/drush si standard \
  --db-url='mysql://user:pass@db:3306/drupal' \
  --site-name='Gitpod Drupal' \
  --account-name=admin --account-pass=admin -y"

echo "--- Enable ECK only ---"
/usr/bin/env docker compose exec -T drupal bash -lc '/opt/drupal/vendor/bin/drush en eck -y'

echo "--- Dev error verbosity ---"
/usr/bin/env docker compose exec -T drupal bash -lc "cat <<'PHP' >> /opt/drupal/web/sites/default/settings.php
...
PHP"
/usr/bin/env docker compose exec -T drupal bash -lc "cat <<'YML' > /opt/drupal/web/sites/development.services.yml
parameters:
...
echo "--- Clear caches ---"
/usr/bin/env docker compose exec -T drupal bash -lc '/opt/drupal/vendor/bin/drush cr'
